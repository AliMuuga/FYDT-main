...
if(dvd){
  ...
  function updateDVD(){
    dvdX += dvdVX;
    dvdY += dvdVY;
    if(dvdX <=0 || dvdX + dvd.offsetWidth >= window.innerWidth){dvdVX*=-1; dvd.style.filter=`drop-shadow(0 0 15px ${getRandomColor()})`;}
    if(dvdY <=0 || dvdY + dvd.offsetHeight >= window.innerHeight){dvdVY*=-1; dvd.style.filter=`drop-shadow(0 0 15px ${getRandomColor()})`;}
...
/* =========================
   HERO VIDEO SWITCH & TEXT (homepage only)
========================= */
...
if(starCanvas){
  ...
  let stars=[], sw, sh;
  function resizeStars(){sw=starCanvas.width=window.innerWidth; sh=starCanvas.height=window.innerHeight;}
  window.addEventListener("resize", resizeStars);
  resizeStars();

  class Star{
    constructor(){this.reset();}
    reset(){this.x=Math.random()*sw; this.y=Math.random()*sh; this.size=Math.random()*1.2; this.speed=Math.random()*0.4+0.05; this.opacity=Math.random(); this.fade=Math.random()*0.02;}
    update(){this.y+=this.speed; this.opacity+=this.fade; if(this.opacity>=1 || this.opacity<=0) this.fade*=-1; if(this.y>sh){this.reset(); this.y=0;}}
    draw(){starCtx.beginPath(); starCtx.arc(this.x,this.y,this.size,0,Math.PI*2); starCtx.fillStyle=`rgba(255,255,255,${this.opacity})`; starCtx.fill();}
  }
...
if(themeIcon){
  themeIcon.addEventListener('click',()=>{document.body.classList.toggle('night');});
}

/* =========================
   MORPHING IMAGES (Models, Posters, Clothing)
========================= */
